...
class invIndexCreator():
    ...
    '''
    def __init__(self, useStemming=True, useStopwords=True):
        self.invertedIndex = {}
        i = 1
        for files in os.listdir(os.path.join("..", "analysis", "separated")):
            words = cleaner.cleanFile("cranfield" + str(i) + ".txt", useStemming=useStemming, useStopwords=useStopwords)
    ...
    '''
    def writeInvIndex(self, fileName):
        with open(fileName, "w") as dadumped:  
